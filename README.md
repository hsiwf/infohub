...
- **活动接龙**：发起接龙生成链接发到班群，同学点开即填即交（无需注册）；粘贴班级名单自动比对，**谁没接龙一目了然**，支持"学号+姓名"匹配重名、名单外标记、专属链接防代填、未接名单一键提醒、导出 CSV、扫码接龙、管理权委托给班委
- **抽签点名**：按班级名单建签箱，抽 N 人参加活动——**抽过的人自动排除，下次不会再被抽到**，箱内抽空自动开始新一轮，保证大家轮流参加；每次抽几人可自定义，支持撤销、重置、历史留痕
- **生日祝福**：🎂 成员生日倒计时，**生日当天专属祝福卡 + 信息页横幅 + 浏览器通知**，一键复制祝福发到班群；名单一键导入，只需补填生日
...
- **省心运维**：每天自动备份数据库（保留 14 份）、JSON 导出 / 导入（含附件记录与接龙数据）、统计面板、**三档外观（浅色 / 深色 / 按时间自动）**、PWA 可安装到手机桌面
...
侧栏「🐉 活动接龙」发起接龙：填标题、粘贴班级名单（支持 Excel / QQ 名单直接粘贴，自动识别"学号 姓名"），生成链接发到班群。
...
让班级更有温度：成员生日倒计时 + 当天祝福提醒。生日面前人人平等——不分老师、同学，每一位成员都会被同样地庆祝。

- **成员**：班级里的每一个人（老师、助理、同学都一样）；生日只存月日，年份选填（填了会显示"将满 N 岁"）；2 月 29 日生日在平年按 2 月 28 日庆祝
- **一键导入**：从名单库选班级名单即可批量导入成员，之后逐个补填生日即可
- **当天祝福**：生日页为寿星展示专属祝福卡片（可自定义祝福语，也可自动生成）；一键复制祝福文案发到班群
- **不错过**：信息页顶部出现生日横幅；开启浏览器通知（顶栏 🔔）后，生日当天弹窗提醒（每 5 分钟检查一次，当天只弹一次）

侧栏「🎂 生日祝福」→ 从名单库导入 → 补填生日，剩下的交给 InfoHub。生日信息对可浏览页面的所有人可见（适合班级共享场景）。
...
| GET / POST | `/api/birthdays` | 生日成员列表（按倒计时排序，含当天标记）/ 添加成员 |
| PUT / DELETE | `/api/birthdays/:id` | 编辑 / 删除成员 |
| POST | `/api/birthdays/import` | 从名单库批量导入生日成员（同名跳过） |
